...
enum SaleType {
  WHITELIST = CollectionConfig.ERC1155.whitelistSale.price,
  PRE_SALE = CollectionConfig.ERC1155.preSale.price,
  PUBLIC_SALE = CollectionConfig.ERC1155.publicSale.price,
}
...
describe(CollectionConfig.ERC1155.contractName, function () {
...
/*     expect(await contract.name()).to.equal(CollectionConfig.ERC1155.tokenName);
    expect(await contract.symbol()).to.equal(CollectionConfig.ERC1155.tokenSymbol); */
    expect(await contract.cost()).to.equal(getPrice(SaleType.WHITELIST, 1));
    expect(await contract.maxSupply()).to.equal(CollectionConfig.ERC1155.maxSupply);
  ...
  it("Whitelist sale", async function () {
    ...
    await contract.setCost(
      utils.parseEther(CollectionConfig.ERC1155.preSale.price.toString())
    );
  ...
  it("Pre-sale (same as public sale)", async function () {
    await contract.setMaxMintAmountPerTx(
      CollectionConfig.ERC1155.preSale.maxMintAmountPerTx
    );
    ...
    await contract.setCost(
      utils.parseEther(CollectionConfig.ERC1155.publicSale.price.toString())
    );
  });
  ...
  it("Supply checks (long)", async function () {
    ...
    const iterations = Math.floor(
      (CollectionConfig.ERC1155.maxSupply - alreadyMinted) / maxMintAmountPerTx
    );
    const expectedTotalSupply = iterations * maxMintAmountPerTx + alreadyMinted;
    const lastMintAmount = CollectionConfig.ERC1155.maxSupply - expectedTotalSupply;
    ...
    for (const i of [...Array(lastMintAmount).keys()].reverse()) {
      expectedWalletOfOwner.push(
        BigNumber.from(CollectionConfig.ERC1155.maxSupply - i)
      );
    }
    expect(
    ...
    expect(await contract.totalSupply()).to.equal(CollectionConfig.ERC1155.maxSupply);
  });
  ...
  it("Token URI generation", async function () {
    ...
    // ERC1155A uses token IDs starting from 0 internally...
